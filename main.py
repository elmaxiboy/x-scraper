...
def get_new_height(driver,last_tweet, last_height):
    ...
    try:
        ...
        for _ in range(5):  # Simulate a user scrolling gradually
...
def extra_security_prompt():
    ...
    try:
        ...
        phone_or_email.send_keys(os.getenv('X_EMAIL'))
...
def login():
    ...
    try:
        ...
        username.send_keys(os.getenv('X_USERNAME'))
                ...
                for tweet in new_tweets:
                    try:
                        ...
                        if tweet.id not in scraped_tweet_ids:
                            ...
                            last_lines="\n".join(tweet_content.splitlines()[-4:])
                            tweet_engagment_numbers.extend(map(int, re.findall(r"\n\s*(\d+)", "\n" + last_lines))) 
...
def logout():
    ...
    try:
        ...
        time.sleep(MAX_DELAY)
        logout = driver.find_element(By.XPATH, f"//span[contains(text(), 'Log out')]")
